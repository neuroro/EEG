...
% (generated by eegTimeFrequency.m or eegTimeFrequency3EventRelated.m),
% which are stored in that folder or in sub-folders of that folder,
% ignoring other .mat files
%
% >> eegLocalSpectra( channels, weighting, imputation )
%
% For example:
% >> eegLocalSpectra
% >> eegLocalSpectra( 'Fz' )
% >> eegLocalSpectra( 11 )
% >> eegLocalSpectra( { 'Fz' 'FCz' 'FFC1' 'FFC2' } )
% >> eegLocalSpectra( [ 11 6 5 12 ], 'weighted', 10 )
%
% .. . .  .   .     .        .             .                     .                                  .                                                       .
%
% •••( Function Inputs )
...
for f = 1:nFiles
    ...
    for w = 1:nCentres
        ...
        if    isempty( Decomposition.(currentCentre).SpectralPower  )             ...
           ...
           || isscalar( Decomposition.(currentCentre).SpectralPower  )            ...
           || isscalar( Decomposition.(currentCentre).PhaseCoherence )            ...
           || isscalar( Decomposition.(currentCentre).Coefficients   )            ...
           || ~any( Decomposition.(currentCentre).SpectralPower,  'all' )         ...
           || ~any( Decomposition.(currentCentre).PhaseCoherence, 'all' )         ...
           || ~any( Decomposition.(currentCentre).Coefficients,   'all' )
