...
% statistic of the cluster permutation tests
%
...
% spectra file(s) from which to compare oscillatory differences
%
% Specify the condition(s) and local spectra file(s) to compare as input
...
% 
% For example:
% >> Stats = spectralClusters;
% >> Stats = spectralClusters( 0.025, 'theta+', 1000, 1e5, 250 );
% >> Stats = spectralClusters( 0.05, [], [], 1e5, 'none', ...
%                              'Active', 'Control', 'TimeFrequencyCluster.mat' );
% >> Stats = spectralClusters( 0.05, [], [], 1e5, 250, 'Correct', 'Correct', ...
%                              'Active.mat', 'Control.mat' );
%
% .. . .  .   .     .        .             .                     .                                  .
%
% •••( Function Inputs )
...
% Local spectra naming for figures and data fields
localSpectraName1 = 'Cluster1';
localSpectraName2 = 'Cluster2';
...
% Conditions
if exist( 'conditionA', 'var' ) && ~isempty( conditionA )
    condition1 = conditionA;
end
if exist( 'conditionB', 'var' ) && ~isempty( conditionB )
...
if exist( 'localSpectraA', 'var' )
    spectra1Name = erase( localSpectra1, { '.mat' 'TimeFrequency' '_' '-' ' ' } );
    spectra2Name = erase( localSpectra2, { '.mat' 'TimeFrequency' '_' '-' ' ' } );
else
    spectra1Name = localSpectraName1;
    spectra2Name = localSpectraName2;
end
...
for iMetric = 1:nSpectralMetrics
    for iEventWindow = 1:nCentres
        ...
        if ~isempty( comparison3 )
            comparison3Title = [ newline comparison3(2:end) ]; % 2:end to remove the space
        end
        ...
        title( figNameGA1 )
        figGA1      = colourMap( figGA1, colourLimits );
        ...
        title( figNameGA2 )
        figGA2      = colourMap( figGA2, colourLimits );
        ...
        if ~isempty( downsampling ) && isnumeric( downsampling ) && downsampling ~= samplingRate
            ...
            if exist( 'eeglab', 'file' )
                ...
                for c = 1:2
                    ...
                    for p = 1:N
                        switch c
                            case 1
                                singleSpectra = squeeze( oscillations1(p,:,:) );
                            case 2
                                singleSpectra = squeeze( oscillations2(p,:,:) );
                        end
                        EEG            = eeg_emptyset();
            ...
            % Downsample without anti-aliasing
                ...
                timeShift   = samplingRate / downsampling;
                dt          = round( timeShift );
                spectra1    = oscillations1(:,:,1:dt:end);
                spectra2    = oscillations2(:,:,1:dt:end);
        ...
        % No downsampling
            ...
            spectra1    = oscillations1;
            spectra2    = oscillations2;
        ...
        testedMeanD = squeeze( mean( spectra1 - spectra2, 1, 'omitnan' ) );
        ...
        % Permutation test
        [ P, Sigma ] = monteCluster( spectra1, spectra2, nPermutations, ...
                                     'tf', pThreshold, [], frequencies, ...
                                     timeShift, 1                       );

        % Store in struct
        Stats.(eventWindow).(metric).P     = P;
        Stats.(eventWindow).(metric).Sigma = Sigma;

        fprintf( '\n' )


        %% Event-related spectra descriptive statistics
        % -----------------------------------------------------------------
        ...
        % Mean of individual event-related spectra
        distributionMean1 = mean( distribution1, 2, 'omitnan' );
        distributionMean2 = mean( distribution2, 2, 'omitnan' );
        Stats.(eventWindow).(metric).Stats.([ Comparison1Field 'Average' ]) ...
            = descriptiveStatistics( distributionMean1 );
        Stats.(eventWindow).(metric).Stats.([ Comparison2Field 'Average' ]) ...
            = descriptiveStatistics( distributionMean2 );

        % Median of clusters extrema of individual event-related spectra
        ...
        if     contains( P.Direction, 'positive', 'IgnoreCase', true )
            data1Values = oscillations1 .* maskPositiveN;
            data2Values = oscillations2 .* maskPositiveN;
        elseif contains( P.Direction, 'negative', 'IgnoreCase', true )
            data1Values = oscillations1 .* maskNegativeN;
            data2Values = oscillations2 .* maskNegativeN;
        elseif contains( P.Direction, { 'no' 'undefined' }, 'IgnoreCase', true )
        ...
        distributionClusters1 = reshape( data1Values, N, nFrequencies * nTimesLimited );
        distributionClusters2 = reshape( data2Values, N, nFrequencies * nTimesLimited );
        distributionClusters1 = median( distributionClusters1, 2, 'omitnan' );
        distributionClusters2 = median( distributionClusters2, 2, 'omitnan' );
        Stats.(eventWindow).(metric).Stats.([ Comparison1Field 'Clusters' ]) ...
            = descriptiveStatistics( distributionClusters1 );
        Stats.(eventWindow).(metric).Stats.([ Comparison2Field 'Clusters' ]) ...
            = descriptiveStatistics( distributionClusters2 );

        % Inter-quartile range of individual event-related spectra
        ...
        % L-scale of individual event-related spectra
        distributionLscale1 = lscale( distribution1 );
        distributionLscale2 = lscale( distribution2 );
        ...
        % Root mean squared deviation from the signed root mean squared of
        % individual event-related spectra
        distributionRMSD1 = rmsdrms( distribution1, 2 );
        distributionRMSD2 = rmsdrms( distribution2, 2 );
        Stats.(eventWindow).(metric).Stats.([ Comparison1Field 'RMSDeviationRMS' ]) = descriptiveStatistics( distributionRMSD1 );
        Stats.(eventWindow).(metric).Stats.([ Comparison2Field 'RMSDeviationRMS' ]) = descriptiveStatistics( distributionRMSD2 );


        %% Plot distributions of event-related spectra
        % -----------------------------------------------------------------
        ...
        % Mean of individual event-related spectra
        figMean = plotDistributions( distributionMean1, distributionMean2, ...
                                     metricUnits, comparison1, comparison2, limits );
        averageName = [ eventWindow '-Related ' bandName newline ...
                        'Distribution of Mean ' metric           ];
        title( averageName )
        if contains( metric, 'coherence', 'IgnoreCase', true )
            xl  = xlim;
            xl1 = max( xl(1), 0 );
        ...
        averageFile = [ eventWindow '-Related ' bandName ' Distribution of Mean ' ...
                        metric ' for ' comparison1 ' vs ' comparison2 comparison3 '.fig' ];
        savefig( figMean, averageFile )

        % Median of clusters extrema of individual event-related spectra
        figC = plotDistributions( distributionClusters1, distributionClusters2, ...
                                  metricUnits, comparison1, comparison2, limits );
        ...
        limitsIQR = [0 maxRange];
        figIQR    = plotDistributions( distributionIQR1, distributionIQR2, ...
                                       metricUnits, comparison1, comparison2, limitsIQR );
        xl    = xlim;
        xl(1) = max( xl(1), 0 );
        xlim( xl )
        ...
        distributionsRMSFile = [ eventWindow '-Related ' bandName ' '         ...
                                 ...
                                 'Deviation for '  comparison1 ' vs '         ...
                                 comparison2 comparison3 '.fig'               ];
...
% L-scale
ls = constant * sum( terms, 2, 'omitnan' );
...
%%
% •.° Root Mean Squared Deviation From The Signed Root Mean Squared °.•
% _________________________________________________________________________
%
...
if ( exist( 'raincloud_plot_k.m', 'file' ) || exist( 'raincloud_plot.m', 'file' ) ) ...
   && ~all( isnan( data1 ), 'all' ) && ~all( isnan( data2 ), 'all' )
    ...
    % Remove negtive y-axis tick marks
    yAxisTicks   = yticks;
    densityTicks = yAxisTicks(yAxisTicks >= 0);
    yticks( densityTicks );
...
% _________________________________________________________________________
%
function fig = colourMap( fig, limits )

if     exist( 'jetzeroed.m', 'file' )
    colormap( jetzeroed( limits ) )

elseif exist( 'colourszeroed.m', 'file' )
    colormap( colourszeroed( limits ) )
