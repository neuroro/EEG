function ...
Stats = spectralClustersGroups( pThreshold, frequencyBand, timeLimits, ...
...
% the maximum cluster sum of Welch's independent samples t-statistics as
% the test statistic of the permutation tests
%
...
% 
% >> spectralClustersGroups( pThreshold, frequencyBand, timeLimits, ...
%                            nPermutations, downsampling,           ...
%                            conditionA, conditionB,                ...
%                            localSpectraA, localSpectraB           )
% 
% For example:
% >> spectralClustersGroups
% >> Stats = spectralClustersGroups( 0.025, 'theta+', [0 1000], 1e5, 250 );
% >> Stats = spectralClustersGroups( 0.05, [], [], 1e5, 'none' );
% >> Stats = spectralClustersGroups( 0.05, [], [], 1e5, 250, 'Active', 'Control' );
%
% .. . .  .   .     .        .             .                     .                                  .
%
% •••( Function Inputs )
...
%
%   monteClusterGroups.m by Rohan King (2023) available from the author
%
%   distanceFunction.m by Rohan King (2023) available from the author
%
%   grow.m by Rohan King (2023) available from the author
%
...
%
% King, R. (2024). spectralClustersGroups [MATLAB code]. GitHub. 
%  https://github.com/neuroro/EEG/spectralClustersGroups.m
...
disp( 'the maximum cluster sum of Welch''s independent samples t-statistic as'  )
disp( 'the test statistic of the permutation tests'                             )
disp( ' ' )
...
for iMetric = 1:nSpectralMetrics
    for iEventWindow = 1:nCentres
        ...
        % Spectra for each group of people
        ...
        % Average spectra differences between people
        oscillationsD = grandAverage1 - grandAverage2;


        %% Spectral difference t-statistics
        % -----------------------------------------------------------------

        % Welch's independent samples t-statistics
        spectraSD1      = squeeze( std( oscillations1, 0, 1, 'omitnan' ) );
        spectraSD2      = squeeze( std( oscillations2, 0, 1, 'omitnan' ) );
        N1              = size( oscillations1, 1 );
        N2              = size( oscillations2, 1 );
        tStats          = oscillationsD ./ sqrt( spectraSD1.^2 / N1 + spectraSD2.^2 / N2 );
        tStats          = squeeze( tStats );

        % Cluster t-threshold
        df              = ( spectraSD1.^2 / N1 + spectraSD2.^2 / N2 ).^2 ...
                          ./                                             ...
                          ( spectraSD1.^4 / ( N1^2 * ( N1 - 1 ) ) +      ...
                            spectraSD2.^4 / ( N2^2 * ( N2 - 1 ) ) );
        ...
        maskedD   = squeeze( oscillationsD ) .* mask;
        ...
        testedMeanD = squeeze( mean( spectra1, 1, 'omitnan' ) - mean( spectra2, 1, 'omitnan' ) );
        ...
        % Permutation test
        [ p, s ] = monteClusterGroups( spectra1, spectra2, nPermutations, ...
                                       'tf', pThreshold, true,            ...
                                       frequencies, timeShift, true       );
        ...
        if     ~isnan( p(1) ) && p(1) == p(2) && p(1) ~= p(3)
            direction = 'Positive difference';
        elseif ~isnan( p(1) ) && p(1) == p(3) && p(1) ~= p(2)
        ...
        % from the signed root mean squared) for the unpaired comparison
        distribution1     = reshape( oscillations1, N1, nFrequencies * nTimesLimited );
        distribution2     = reshape( oscillations2, N2, nFrequencies * nTimesLimited );
        ...
        maskPositiveN1 = zeros( N1, nFrequencies, nTimesLimited );
        maskNegativeN1 = zeros( N1, nFrequencies, nTimesLimited );
        maskN1         = zeros( N1, nFrequencies, nTimesLimited );
        maskPositiveN2 = zeros( N2, nFrequencies, nTimesLimited );
        maskNegativeN2 = zeros( N2, nFrequencies, nTimesLimited );
        maskN2         = zeros( N2, nFrequencies, nTimesLimited );
        for pn = 1:N1
            maskPositiveN1(pn,:,:) = maskPositive;
            maskNegativeN1(pn,:,:) = maskNegative;
            maskN1(pn,:,:)         = mask;
        end
        for pn = 1:N2
            maskPositiveN2(pn,:,:) = maskPositive;
            maskNegativeN2(pn,:,:) = maskNegative;
            maskN2(pn,:,:)         = mask;
        end
        if p(1) == p(2) && ~isnan( p(1) )                                   % ~isnan( p(2) ) && ( p(2) < p(3) || isnan( p(3) ) )
            data1Values = oscillations1 .* maskPositiveN1;
            data2Values = oscillations2 .* maskPositiveN2;
        elseif p(1) == p(3) && ~isnan( p(1) )                               % ~isnan( p(3) ) && ( p(3) < p(2) || ~isnan( p(2) ) )
            data1Values = oscillations1 .* maskNegativeN1;
            data2Values = oscillations2 .* maskNegativeN2;
        elseif isnan( p(1) )                                                % ( isnan( p(2) ) && isnan( p(3) ) )
            data1Values = oscillations1 .* NaN;
            data2Values = oscillations2 .* NaN;
        else
            data1Values = oscillations1 .* maskN1;
            data2Values = oscillations2 .* maskN2;
        end
...
Stats.Comparison.DifferenceType          = 'Unpaired data from parallel measurement of separate groups';
...
Stats.Comparison.ClusterStatistic        = 'Maximum cluster sum of Welch''s independent samples t-statistics'; 
